...
namespace CosmosDb.ServerSide.Demos
{
  public static class StoredProceduresDemo
  {
    ...
    private async static Task CreateStoredProcedure(string sprocId)
    {
      ...
      var container = Shared.Client.GetContainer("FamiliesDB", "Families");
    ...
    private async static Task Execute_spHelloWorld()
    {
      ...
      var scripts = Shared.Client.GetContainer("FamiliesDB", "Families").Scripts;
    ...
    private async static Task Execute_spSetItaly()
    {
      Console.WriteLine();
      Console.WriteLine("Execute spSetItaly");
      ...
      var container = Shared.Client.GetContainer("FamiliesDB", "Families");
      var pk = new PartitionKey(documentDefinition.location.state);
      var result = await container.Scripts.ExecuteStoredProcedureAsync<dynamic>(
        "spSetItaly",
        pk,
        new[] { documentDefinition, true });
      var document = result.Resource;

      var country = document.location.state;
      var isIT = document.location.isItaly;
